...
// Ask Gemini for a handful of authentic, well-known dishes from a given
// cuisine/country. Used as a fallback when TheMealDB has zero recipes for
// that area, so no country on the dashboard is ever a total dead end.
// This only returns lightweight suggestions (name + description) — the
// full recipe (ingredients, steps, nutrition) is generated on demand when
// the user actually clicks one, via the existing getOrGenerateRecipe flow.
export async function suggestCuisineDishes(cuisineName) {
  try {
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash-lite" });

    const prompt = `
List 6 authentic, well-known traditional dishes from ${cuisineName} cuisine.

Return ONLY valid JSON, no markdown, no code fences, in this exact format:
[
  { "name": "Dish Name", "description": "One short sentence describing the dish." }
]

Rules:
- Dishes must be real, traditional dishes actually associated with ${cuisineName} cuisine.
- Keep descriptions under 15 words.
- Return exactly 6 dishes.
`;
    ...
    console.error("Error suggesting cuisine dishes:", error);
    return {
      success: false,
      dishes: [],
    };
